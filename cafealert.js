        ...
        if(0 + response.select('article:first-child > articleid').text() > articleNum){
            articles.forEach(element => {
                ...
                a += '게시판: ' + org.jsoup.parser.Parser.unescapeEntities(element.select('menuname').text(), true) + '\n'
                  + '제목: ' + org.jsoup.parser.Parser.unescapeEntities(element.select('subject').text(), true) + '\n'
                  + '닉네임: ' + org.jsoup.parser.Parser.unescapeEntities(element.select('nickname').text(), true) + '\n'
                  + '아이디: ' + org.jsoup.parser.Parser.unescapeEntities(element.select('writerid').text(), true) + '\n'
                  + '읽음: ' + element.select('readCount').text() + '명\n'
                  + '댓글: ' + element.select('commentCount').text() + '개\n'
                  + element.select('writedate').text() + '\n'
                  + '\n';
            });
            Bot.send(Room, a, KTPackage);
            articleNum = Number(response.select('article:first-child > articleid').text());
        }
    } catch (e) {
        Log.e(e);
    }
